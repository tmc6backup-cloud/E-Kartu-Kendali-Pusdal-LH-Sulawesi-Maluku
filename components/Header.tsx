    ...
    const fetchNotif = async () => {
        ...
        try {
            ...
            if (isValidatorRole(user.role)) {
                ...
                if (myQueue.length > 0) {
                    newNotifs.push({ id: 'q', title: 'Antrian Baru', desc: `${myQueue.length} berkas menunggu.`, icon: <Clock className="text-amber-500" /> });
                }
            }
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-40 no-print">
            <div className="flex items-center gap-6 flex-1">
                ...
                <div className="relative max-w-md w-full hidden sm:block">
                    ...
                    <input type="text" placeholder="Cari berkas..." className="w-full pl-10 pr-4 py-2 bg-slate-100 rounded-xl text-sm outline-none focus:bg-white focus:border-blue-500 transition-all" />
                </div>
            </div>
            ...
            <div className="flex items-center gap-6">
                <div className="relative" ref={notificationRef}>
                    ...
                    {isNotificationsOpen && (
                        <div className="absolute right-0 mt-3 w-80 bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden z-50">
                            <div className="p-4 bg-slate-50 border-b font-bold text-xs uppercase tracking-widest">Pusat Informasi</div>
                            <div className="max-h-60 overflow-y-auto">
                                {notifications.length > 0 ? notifications.map(n => (
                                    <div key={n.id} className="p-4 hover:bg-slate-50 cursor-pointer flex gap-3 border-b border-slate-100 last:border-0">
                                        {n.icon}
                                        <div>
                                ...
                                )) : <div className="p-8 text-center text-[10px] text-slate-400 uppercase font-bold">Tidak ada update</div>}
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="text-right hidden sm:block">
                        ...
                        <p className="text-[9px] text-blue-600 font-black uppercase tracking-tighter">{user?.role?.replace(/_/g, ' ')}</p>
                    </div>
